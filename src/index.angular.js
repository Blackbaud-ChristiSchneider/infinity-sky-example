...
(function () {
    ...
    angular.module('example', ['sky', 'ui.bootstrap', 'ui.router', 'ngAnimate', 'example.exampleApi', 'example.templates', 'example.resources', 'Debugging', ])
        .config(authInterceptorConfig)
        .config(config)
        .run(['bbWindowConfig', '$state', 'crm.rootFolder', function (bbWindowConfig, rootFolder) {
            ...
            bbWindowConfig.productName = rootFolder;
    
        }])
        .controller('MainController', MainController);

    authInterceptorConfig.$inject = ["$httpProvider", 'exampleApiProvider'];
    function authInterceptorConfig($httpProvider, exampleApiProvider) {
        var exampleApi = exampleApiProvider.$get();
    ...
    MainController.$inject = ['$scope', '$document', '$timeout', 'Routing', 'bbWait', 'frogApi', 'frogResources', '$location', 'crm.rootFolder', 'crm.isCustomApp'];

    function MainController($scope, $document, $timeout, Routing, bbWait, frogApi, frogResources, $location, rootFolder, isCustomApp) {
        var self = this;
        ...
        function setUpInactiveLogOut(inactiveTimeoutSeconds) {
            ...
            LastActivityStorageKey = "BB_LastActivity_" + fullUrl.substring(0, fullUrl.indexOf("/sky/" + (isCustomApp ? 'custom/' : '') + rootFolder));
                ...
                frogApi.logoutAsync()
                    .then(function (logoutWasSuccessful) {
                        if (logoutWasSuccessful) {
                            ...
                            url = (isCustomApp ? '../' : '') + "../../webui/WebShellLogin.aspx?databaseName=" + euc(frogApi.getDatabaseName());
